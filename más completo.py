...
imgFondo = pygame.image.load("fondo.png")
        imgUAIBOT = pygame.transform.scale(pygame.image.load("UAIBOT.png").convert_alpha(), (GRID_SIZE, GRID_SIZE))
imgUAIBOTA = pygame.transform.scale(pygame.image.load("UAIBOTA.png").convert_alpha(), (GRID_SIZE, GRID_SIZE))
            imgUAIBOTINO = pygame.transform.scale(pygame.image.load("UAIBOTINO.png").convert_alpha(), (GRID_SIZE, GRID_SIZE))
imgUABOTINA = pygame.transform.scale(pygame.image.load("UAIBOTINA.png").convert_alpha(), (GRID_SIZE, GRID_SIZE))
imgGanaste = pygame.image.load("imgGanaste.png")
            imgPerdiste = pygame.image.load("imgPerdiste.png")
# Cargar imágenes de las bolsas
imgBolsaVerde = pygame.transform.scale(pygame.image.load("BolsaVerde.png").convert_alpha(), (GRID_SIZE, GRID_SIZE))
imgBolsaGrisOscuro = pygame.transform.scale(pygame.image.load("BolsaGrisOscuro.png").convert_alpha(), (GRID_SIZE, GRID_SIZE))
imgCestoNegro = pygame.transform.scale(pygame.image.load("cestonegro.png").convert_alpha(), (GRID_SIZE, GRID_SIZE))
imgCestoVerde = pygame.transform.scale(pygame.image.load("cestoverde.png").convert_alpha(), (GRID_SIZE, GRID_SIZE))
...
def cambiarPersonaje():
    ...
    if personajeActual == 'UAIBOT':
        ...
        time.sleep(0.1)
def reiniciar_juego():
    global personajeActualIndex, personajeActual, avatar, avatar_rect, cargaActual, contadorcestoVer, contadorcestoGri, bolsas, objetivoGanar
    personajeActualIndex = 0
    personajeActual = personajes[personajeActualIndex]
    avatar = imgUAIBOT
    avatar_rect.x = 100
...
def dibujarBasuraYTachos():
    ...
    return False      
def cartelCestosVer():
    dibujarTexto(f'Cantidad de bolsas: {contadorcestoVer}', tipografia, colorBlanco, 0, 0, colorAzul , 895,175)
def cartelCestosGri():
    dibujarTexto(f'Cantidad de bolsas: {contadorcestoGri}', tipografia, colorBlanco, 0, 0, colorAzul , 895,375)
...
while juegoEnEjecucion:
        ...
        if not bolsa['cargada'] and avatar_rect.colliderect(bolsa['rect']):
            if (cargaActual[personajeActual]['verdes'] + cargaActual[personajeActual]['grises']) < capacidades[personajeActual]:
                ...
                if bolsa['tipo'] == 'verde':
                    ...
                    pygame.mixer.Channel(2).play(pygame.mixer.Sound("agarrar bolsa.wav"))
    ...
    if cargaActual[personajeActual]['grises'] == 0 and cargaActual[personajeActual]['verdes'] == 0:
        if contadorcestoVer == 2 and contadorcestoGri == 3:
            ...
            pygame.mixer.Channel(3).play(pygame.mixer.Sound("sonido Ganar.wav"))
            pygame.display.flip()
    ...
    if val:
        if tiempoLimite > 0:
            tiempoLimite -= 1 / 60
            dibujarTexto(f'Tiempo: {tiempoLimite:.2f} segundos', tipografia, colorBlanco, 200, 30, colorNegro, 15, 60)
        else:
            tiempoLimite = 0  # Ensure it doesn't go below 0
            dibujarTexto(f'Tiempo restante: 0.00 segundos', tipografia, colorBlanco, 200, 30, colorNegro, 15, 60)
